...
/*   Updated: 2025/03/07 21:52:35 by fifrandr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_light_list	*create_light_node(char *line)
{
	t_light_list	*new_node;

	new_node = malloc(sizeof(t_light_list));
	if (!new_node)
		return (NULL);
	new_node->light = malloc(sizeof(t_light));
	if (!new_node->light)
	{
		free(new_node);
		return (NULL);
	}
	if (!parse_light(line, new_node->light))
	{
		free(new_node->light);
		free(new_node);
		return (NULL);
...
int	add_light(t_scene *scene, char *line)
{
	t_light_list	*new_node;
	t_light_list	*temp;

	new_node = create_light_node(line);
	if (!new_node)
		return (0);
	if (!scene->light)
		scene->light = new_node;
	else
	{
		temp = scene->light;
		while (temp->next)
